...
then
  ...
  echo "Usage: run-client.sh [cheetah|SCI_HE|hare] [sqnet/resnet50]"
else
  if ! contains "cheetah SCI_HE hare" $1; then
    echo -e "Usage: run-client.sh ${RED}[cheetah|SCI_HE|hare]${NC} [sqnet|resnet50|densenet121]"
	exit 1
  fi

  if ! contains "sqnet resnet50 densenet121" $2; then
    echo -e "Usage: run-client.sh [cheetah|SCI_HE|hare] ${RED}[sqnet|resnet50|densenet121]${NC}"
	exit 1
  fi
